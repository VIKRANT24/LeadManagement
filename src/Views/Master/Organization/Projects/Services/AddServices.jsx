...
const AddServices = ({ selectedServiceId, newProjectId, cancle }) => {
  ...
  const isFormValid = inputdata.serviceName && inputdata.status; // Check if the required fields are filled
          ...
          try {
            ...
            if (response.status === 200) {
              if (response?.data?.success) {
                ...
                const fetchedData = response?.data?.data?.service;
            ...
            if (response?.status !== 404) {
              toast.error(
                "An error occurred while fetching service data. Please try again."
              );
  ...
  const validateForm = () => {
    ...
    if (!inputdata.serviceName) {
      newErrors.serviceName = "Enter your service Name";
    }
    if (!inputdata.status) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!selectedServiceId) {
        response = await postService(serviceProjectId, inputdata);
      } else {
        response = await updateServiceById(
          serviceProjectId,
          selectedServiceId,
          inputdata
        );
      ...
      if (
        ...
        //   () =>
        //     navigate(
        //       `/admin/master/organization/projects/${organizationId}/services/${serviceProjectId}`
        //     ),
        //   3000
        ...
        toast.error("Failed to save the service. Please try again.");
      }
    } catch (error) {
                ...
                <Form className="mt-3">
                  <Row>
                    <Col className="pr-1" md="6">
                      <FormGroup>
                        <label>
                          SERVICES NAME <span className="star">*</span>
                        </label>
                        <Input
                          style={{
                            borderColor: errors.serviceName ? "red" : "#90909A",
                          }}
