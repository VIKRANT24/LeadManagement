...
import { salutations } from "../../../Variables/commonData";

function AddReference() {
  const navigate = useNavigate();
  const { referenceId } = useParams(); // Extract ReferenceId from URL parameters
  ...
  const handleInputChange = (e) => {
    ...
    //   // Parse value as integer, but ensure it's a number
    //   const parsedValue = value ? parseInt(value.replace(/\D/g, ""), 10) : 0; // default to 0 if empty or invalid
    //   setInputdata((prev) => {
    ...
    if (!hasFetched) {
      if (referenceId) {
        ...
        const fetchOrganizations = async () => {
          try {
            const response = await getReferenceById(referenceId); // Await the asynchronous call

            if (response.status === 200) {
              if (response?.data?.success) {
                setIsLoading(false);
                //console.log("Role Data", response?.data?.data?.role);
                const fetchedData = response?.data?.data?.reference;
                setInputdata({
            ...
            if (response?.status !== 404) {
              toast.error(
                "An error occurred while fetching reference data. Please try again."
              );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!referenceId) {
        response = await postReference(payloadData);
      } else {
        response = await updateReferenceById(referenceId, payloadData);
      }
      ...
      if (
        ...
        setTimeout(() => navigate("/admin/lead/reference"), 3000); // Navigate after showing the toast
      } else {
        toast.error("Failed to save the Reference. Please try again.");
      }
    } catch (error) {
