...
import { bgColor } from "../../../Variables/commonData.js";

function AddLeadsStatus({ selectedProjectId, selectedleadStatusId, cancle }) {
  const navigate = useNavigate();
  const [projectId, setProjectId] = useState(selectedProjectId); // To prevent multiple API calls
  const [leadStatusId, setLeadStatusId] = useState(selectedleadStatusId);
  ...
  const isFormValid = inputData.leadStatus && inputData.status; // Check if the required fields are filled
  ...
  useEffect(() => {
    if (!hasFetched) {
      if (leadStatusId) {
        setIsLoading(true);
        setTitle("Edit Lead Status");
        const fetchLeadSource = async () => {
          try {
            const response = await getLeadsStatusById(leadStatusId); // Await the asynchronous call

            if (response.status === 200) {
              if (response?.data?.success) {
                setIsLoading(false);
                //console.log("Role Data", response?.data?.data?.role);
                const fetchedData = response?.data?.data?.leadStatus;
                setInputData({
                  leadStatus: fetchedData?.leadStatus || "",
            ...
            toast.error(
              "An error occurred while fetching lead status data. Please try again."
            );
            console.error("Error:", error);
          }
        };
  ...
  const validateForm = () => {
    ...
    // Validate email ID
    if (!inputData.leadStatus) {
      newErrors.leadStatus = "Enter your lead status";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!leadStatusId) {
        ...
        response = await postLeadsStatus(payload);
      } else {
        ...
        response = await updateLeadsStatusById(leadStatusId, payload);
      }
      ...
      if (
        ...
        toast.success("Leads status saved successfully!");
        // setTimeout(() => navigate("/admin/lead/leadsStatus"), 3000); // Navigate after showing the toast
        setTimeout(() => {
          cancle(); // This function will close the modal and reset the selected ID if needed
        }, 2000);
      } else {
        toast.error("Failed to save the leads status. Please try again.");
      }
    } catch (error) {
                ...
                <Form className="mt-3">
                  <Row>
                    ...
                    <Col className="pr-1" md="12">
                      <FormGroup>
                        <label>
                          LEAD STSTUS <span className="star">*</span>
                        </label>
                        <Input
